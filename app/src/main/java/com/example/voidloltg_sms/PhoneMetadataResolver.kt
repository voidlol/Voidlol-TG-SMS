...
object PhoneMetadataResolver {
    fun resolveSimPhoneNumber(context: Context, intent: Intent): String {
        ...
        if (!hasAnyPhonePermission(context)) {
            return defaultLabel
        }
        ...
        val info = runCatching { subscriptionManager.getActiveSubscriptionInfo(subscriptionId) }.getOrNull()
            ?: return defaultLabel
    ...
    private fun hasAnyPhonePermission(context: Context): Boolean {
